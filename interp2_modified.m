function [zi,s,t,onemt,ndx,nrows] = interp2_modified(varargin)
%
...
if (nargin > 1)
    ...
    if strncmpi(method,'*',1) % Direct call bypass.
        ...
        if strcmpi(method(2),'l') || strcmpi(method(2:4),'bil') 
            ...
            zi = linear(ExtrapVal, varargin{1:end-index}); %here first
            return
...
% .. In Matlab, error([]) is ignored, in Octave that gives a warning
%    So surround it in an if-statement
if any(msg)
  error(msg);
end
...
if strncmpi(method,'l',1) || strncmpi(method,'bil',3) % bilinear interpolation.
    [zi,s,t,onemt,ndx,nrows] = linear(ExtrapVal,x,y,z,xi,yi); % here second
...
%------------------------------------------------------
function [F,s,t,onemt,ndx,nrows] = linear(ExtrapVal,arg1,arg2,arg3,arg4,arg5)
